...
/**
 * 采用 key-value ，先对key进行加工用来保证消息的顺序性 由于再三的思考，考虑到RocksDB的的特性（原理），key最好设定成String 此处的加工方法我选择根据key的类型级别进行String化，然后再加上时间戳
 */
@Service
public class MessageStorageStructure {
    ...
    public boolean sycSaveMessage(KeyMessage<String, Object> keyMessage) {
        ...
        try {
            ...
            Callable<Boolean> save = () -> fastDB
                .putObject(final_key, keyMessage, keyMessage.getTopic_name(), true);
     ...
     */
    public void getMessageAndPutIntoQueue(String topic_patition,
        Queue<KeyMessage<String, Object>> messageQueue) {
        ...
        while (cn.compareTo(tn) != 1) {
            try {
                messageQueue.add((KeyMessage<String, Object>) fastDB
                    .getObject(topic_patition.split("_")[0], topic_patition + (cn.toString())));
            } catch (IOException e) {
                e.printStackTrace();
