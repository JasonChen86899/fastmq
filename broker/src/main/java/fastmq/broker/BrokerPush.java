...
public class BrokerPush extends Thread {
  ...
  /**
   * public BrokerPush(String tcpAddress, int t, Queue messageQueue){//type 指的是ZMQ下面的传输方式 this.mq =
   * messageQueue; this.flag = true; this.tcp = tcpAddress; this.type = t; context =
   * ZMQ.context(1);//可以分配两个线程给给这个context，主要针对两个线程一个分别是消费者，一个是生产者,当然也可以不这样做表示  启动两个在这样的线程，之间可以实行持久化
   * this.transfer = context.socket(type); transfer.bind(tcp); }
   *
   * public BrokerPush(String tcpAddress, int t, String topicname, Queue messageQueue){//type
   * 指的是ZMQ下面的传输方式 this.topicName = topicname; this.mq = messageQueue; this.flag = true; this.tcp =
   * tcpAddress; this.type = t; context = ZMQ.context(1); this.transfer = context.socket(type);
   * transfer.bind(tcp); }
   **/

  public BrokerPush(ZkClient zkClient, String topic_patition, Queue messageQueue, RecordsUtil rec)
      throws Exception {
  ...
  /**
   * private boolean doSend(String sendData){ try{ return transfer.send(sendData); }catch (Throwable
   * throwable) { return false; } }
   *
   * private boolean doSubSend(String sendDate,String topicName){ try{ return
   * (transfer.sendMore(topicName) && transfer.send(sendDate)); }catch (Throwable throwable){ return
   * false; } }
   **/
  public void run() {
    if (type == ZMQ.PUSH) {
  ...
  private void sameTopicGroupPub(ZkClient zkClient, String topic_patition, String topic) {
    ...
    groupChildren.forEach(group ->
        zkClient.subscribeChildChanges("/Consumer/Group/" + group + "/ids", (s, list) -> {
          ConsumerRule.DefalutConsumerRule(topicName,
              new ConsumerGroup(zkClient, "/Consumer/Group/" + group + "/ids", group));
          try {
            HashMap<String, List<String>> consumerip_List_topicPatition = (HashMap<String, List<String>>) SerializationUtil
                .deserialize(zkClient.readData("Consumer/Group/" + group + "/collateMap"));
            groupConsumerIpAddress.put(group,
                consumerip_List_topicPatition.entrySet().stream().findFirst().filter((entry) ->
