...
/** 关于RocksDB的一些说明：
 * RocksDB 最好的支持是在Linux上，当然也能支持在Windows上，两者都需要配置JVM
 */
@Configuration
public class FastDB {
    /**
    ...
    **/
    ...
    public  RocksDB openDatabase(){
        ...
        db = null;
        try{
            db = RocksDB.open(options,"path/fastdb");// All of the contents of database are stored in this directory
    ...
    public Object getObject(String columFamily_topic_name, Object key) throws IOException, RocksDBException {
        ByteArrayOutputStream byteArray = new ByteArrayOutputStream();
        Hessian2Output hessian2Output = new Hessian2Output(byteArray);
        hessian2Output.writeObject(key);
        byte[] keyBytes = byteArray.toByteArray();
        ...
        if(columnFamily_Name_Map.containsKey(columFamily_topic_name)){
            ...
            return null;
        */
        ColumnFamilyHandle cf= db.createColumnFamily(new ColumnFamilyDescriptor(columFamily_topic_name.getBytes()));
