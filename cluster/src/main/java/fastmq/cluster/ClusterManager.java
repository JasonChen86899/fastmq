...
public class ClusterManager extends Thread {
    ...
    private static void SubscribeMQServerNodesChanges(){
        IZkChildListener iZkChildListener = (String var1, List<String> var2) -> {
            //MQServerNodes = var2;
            List<String> aliveMQServernodes = ClusterManager.getMQServerNodes();
            //if(aliveMQServernodes.stream().filter((a) -> a.equals()).findFirst();){
                //return;
            //}

            String ipAddress = aliveMQServernodes.get(new Random(aliveMQServernodes.size()-1).nextInt());
